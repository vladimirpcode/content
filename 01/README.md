...
***
Код в данном туториале не претендует на хороший стиль. Ради простоты .h файлы не используются. Обсуждение .h файлов выходит за рамки туториала.
***
...
Сначала мы определяем общие части (блоки / сущности), а потом идем в сторону уточнения до тех пор пока не останутся сами цифры, числа, переменные, знаки и скобки. Последние назовем терминалами или токенами, а общие понятия нетерминалами. 
В нотации РБНФ | означает "ИЛИ", а {фигурные скобки} повторение элемента ноль или более раз.
И так:
```
...
void next_char(){
    ...
    // если символ не пробел, значит он нам и нужен
}
```
Тоже самое на C++:
```c++
...
Идентификатор = Буква {Буква}
```
Каждому нетерминалу будет соответствовать процедура. Хотя для простых нетерминалов мы отдельную процедуру программировать не будем. На момент вызова процедуры текущей лексемой должна быть первая лексема нетерминала, который эта процедура распознаёт. А после окончания работы процедуры текущей лексемой будет лексема, следующая за распознанным нетерминалом:
```c++
// Выражение = Слагаемое {ОперСлож Слагаемое}
void expression(){
    term();     //слагаемое
